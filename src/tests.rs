...
fn test_add() {
    ...
    assert_eq!(expr.expect_num().into_owned(), 12);
}
#[test]
fn test_parens() {
    let mut env = Env::new();
    let expr = read_eval_print("(10)", &mut env).unwrap();
    assert_eq!(expr.expect_num().into_owned(), 10);
}
#[test]
fn test_embed_parens() {
    let mut env = Env::new();
    let expr = read_eval_print("(10 + 5) * 2", &mut env).unwrap();
    assert_eq!(expr.expect_num().into_owned(), 30);
}
#[test]
fn test_var_read() {
    let mut env = Env::new();
    ...
    assert_eq!(expr.expect_num().into_owned(), 5);
}
#[test]
fn test_var_assign() {
    use env::Env;
    ...
    let num: i32 = env.lookup_var("x").unwrap().0.expect_num().into_owned();
    assert_eq!(num, 2i32)
}